...
export function getTargetRepository(gitAPI: GitAPI, sourceControlOrRepo?: any): GitRepository | undefined {
  ...
  if (activeEditor) {
    ...
    const match = gitAPI.repositories.find((r) => activeUri.startsWith(r.rootUri.toString()));
    if (match) {
      return match;
    }
  }
...
export async function getRepositoryDiff(
  ...
  const hasStaged = repo.state.indexChanges && repo.state.indexChanges.length > 0;
  ...
  if (includeUnstagedIfNoStaged) {
    const hasWorkingTree = repo.state.workingTreeChanges && repo.state.workingTreeChanges.length > 0;
    if (hasWorkingTree) {
