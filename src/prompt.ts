...
export function buildCommitPrompt(diff: string, customPrompt?: string): ChatMessage[] {
  let truncatedDiff = diff;
  if (diff.length > MAX_DIFF_LENGTH) {
    truncatedDiff = diff.slice(0, MAX_DIFF_LENGTH) + '\n\n[... diff truncated due to length ...]';
  }
  ...
  if (customPrompt && customPrompt.trim().length > 0) {
    systemInstructions.push('', 'Additional User Instructions:', customPrompt.trim());
  }
